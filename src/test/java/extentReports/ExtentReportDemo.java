package extentReports;

import com.aventstack.extentreports.ExtentReports;
import com.aventstack.extentreports.ExtentTest;
import com.aventstack.extentreports.Status;
import com.aventstack.extentreports.reporter.ExtentSparkReporter;

public class ExtentReportDemo {
	public static void main(String[] args) {
		ExtentSparkReporter htmlReporter = new ExtentSparkReporter(System.getProperty("user.dir") + "\\target\\resources\\reports" + "\\reports.html");
		
		ExtentReports extent = new ExtentReports();
		extent.attachReporter(htmlReporter);
		
		ExtentTest test1 = extent.createTest("verifyTestForPayment", "Verification for total payment");
		
		test1.assignAuthor("Santosh", "Ajay");
		test1.assignCategory("Smoke", "Regression");
		
		test1.log(Status.INFO, "This is first step");
		test1.log(Status.INFO, "This is second step");
		test1.log(Status.INFO, "This is third step");
		
		test1.info("This is info line appered in reports");
		
		test1.pass("Sample pass test step1");
		test1.pass("Sample pass test step2");
		test1.pass("Sample pass test step3");
		
		test1.fail("Sample FAILED test step");
		
		extent.flush();
		
	}
}
